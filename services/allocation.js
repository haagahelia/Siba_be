...
const updateAllocSubjectPriority = (subjectId, allocRound, priorityNumber) => {
    const sqlQuery = `UPDATE AllocSubject
                        ...
                        ;`
    return new Promise((resolve, reject) => {
        db.query(sqlQuery, [priorityNumber, subjectId, allocRound], (err, result) => {
            if (err) {
                return reject(err);
            } else {
                resolve(result)
...
const findRoomsForSubject = (allocRound, subjectId) => {
    const sqlQuery = `
                    ...
                    FROM Space sp
                    WHERE sp.personLimit >= (SELECT groupSize FROM Subject WHERE id=${db.escape(subjectId)})
...
const setMissingItemAmount = (subjectId, spaceId, missingItemAmount) => {
    ...
    return new Promise((resolve, reject) => {
        db.query(sqlQuery, [missingItemAmount, subjectId, spaceId], (err, result) => {
            if(err){
                return reject(err)
            } else{
                resolve(result)
